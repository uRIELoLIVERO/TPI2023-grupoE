...
def actualizarSubtotal(request, product_id):
    if request.method == 'POST' and request.is_ajax():
        # Obtener el producto desde la base de datos
        producto = get_object_or_404(Producto, id=product_id)

        # Actualizar la cantidad del producto en el carrito
        nueva_cantidad = int(request.POST.get('quantity'))
        producto.cantidad = nueva_cantidad
        producto.subtotal = producto.precioUnitario * nueva_cantidad
        producto.save()

        return JsonResponse({'success': True, 'new_subtotal': producto.subtotal})
    else:
